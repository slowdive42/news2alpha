...
def main():
    ...
    RAW_NEWS_PATH = os.path.join(RAW_NEWS_DIR, f"{QUERY}_{FROM_DATE}_{TO_DATE}.json")
    MARKET_DATA_PATH = os.path.join(MARKET_DATA_DIR, f"{TICKER}_{FROM_DATE}_{TO_DATE}.csv")
    ...
    FINAL_OUTPUT_PATH = os.path.join(FINAL_FEATURES_DIR, f"final_{TICKER}_{FROM_DATE}_{TO_DATE}.csv")

    print("--- Starting News2Alpha Pipeline ---")
    print(f"Configuration: Query='{QUERY}', Ticker='{TICKER}', From='{FROM_DATE}', To='{TO_DATE}'")
    ...
    print("\nStep 1.1: Fetching news...")
    api_key = os.getenv("NEWS_API_KEY")
    if not api_key:
        raise ValueError("NEWS_API_KEY not found in .env file. Please add it.")
    fetch_news(api_key, QUERY, FROM_DATE, TO_DATE, RAW_NEWS_PATH)

    print("\nStep 1.2: Fetching market data...")
    fetch_market_data(TICKER, FROM_DATE, TO_DATE, MARKET_DATA_PATH)
